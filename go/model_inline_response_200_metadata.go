...
// Metadata of the disclosure request
type InlineResponse200Metadata struct {
	// Status of the disclosure requests.  1) \"OPEN\"     : Requests which are yet to be responded by the client/intermediary. 2) \"PENDING\"  : Requests which are responded by the client/intermediary with the intermediaries information but yet to     provide the final responses.      disclosureType “INTERMEDIARIES” received; disclosureType “RESPONSE” not received; or disclosureType “RESPONSE” received but with errors
	Status string `json:"status"`

	RequestMode *InlineResponse200MetadataRequestMode `json:"requestMode"`
	// The date and time at which the request originated/created by the issuer/agent.  Note: This value will be populated only when the disclosure request is created on Broadridge's Shareholder Disclosure Hub.
	RequestOriginTimestamp time.Time `json:"requestOriginTimestamp,omitempty"`
	// The date and time when the Hub first received the request for the specific Hub participant/organization.
	RequestReceiptTimestamp time.Time `json:"requestReceiptTimestamp"`
	// Hub calculated date and time by when Hub participant must supply underlying intermediary client information, if present; typically same-day.
	ForwardRequestDeadlineDate time.Time `json:"forwardRequestDeadlineDate,omitempty"`
	// Hub calculated date and time by when the Hub participant must supply all required shareholder information
	ResponseDeadlineDate time.Time `json:"responseDeadlineDate"`
	// The identification of entity from which request is received (typically issuer or issuer agent or upstream intermediary)
	Sender *AnyOfinlineResponse200MetadataSender `json:"sender"`
}
